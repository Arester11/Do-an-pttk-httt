...
function renderProduct(product) {
    ...
    if(product.salePercent) {
        product.salePrice = ((1 - product.salePercent/100)*product.prePrice).toFixed(2);
        priceHTML = `
...
function controlPaginIconDisplay() {
    ...
    if(totalPage == 1 && current_page == 1) {
        document.querySelector(".pagination").style.display = 'none';
        listProduct.style.paddingBottom = "10px";
    }
    else {
        document.querySelector(".pagination").style.display = 'flex';
        listProduct.style.paddingBottom = 0;
    }
...
cartButton.addEventListener("click", e => {
    ...
    document.querySelector(".product-detail__layout").click();
    reload();
});